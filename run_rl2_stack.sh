...
# Start/restart TopFull RL2 stack on master node:
# - proxy
# - RL2 controller (with per-API network fault compensation)
# - metric collector
...
push_runtime_file "${MASTER_REPO_DIR}" "TopFull_master/online_boutique_scripts/src/deploy_rl2.py"
...
push_runtime_file "${MASTER_REPO_DIR}" ".env"
...
tmux new-session -d -s topfull-controller "cd '${src_dir}' && PYTHONUNBUFFERED=1 python3 deploy_rl2.py > /tmp/topfull-controller.log 2>&1"
