...
# Fault injection with RL controller as mitigation (no manual REJECT).
# Assumes `make rl` was already run — does NOT stop the stack at end.
...
PHASE_LOG="${SCRIPT_DIR}/phase_markers_rl.env"
...
cleanup() {
  echo "$(ts) Cleanup: clearing netem only (leaving RL stack running)..."
  bash "${SCRIPT_DIR}/net_delay_k8s.sh" clear 2>/dev/null || true
  bash "${SCRIPT_DIR}/stop_loadgen_only.sh" 2>/dev/null || ssh node3 'tmux kill-server 2>/dev/null; pkill -f locust 2>/dev/null' 2>/dev/null || true
}
trap cleanup EXIT INT TERM

echo "$(ts) === RL Mitigation Test (POD_COUNT=${POD_COUNT}, ${NET_LOSS_PCT}% loss) ==="
: > "${PHASE_LOG}"
cat >> "${PHASE_LOG}" <<EOF
BASE_RATE=${BASE_RATE}
...
echo "$(ts) warmup ${BASE_SETTLE}s (RL controller learning)..."
sleep "${BASE_SETTLE}"
...
echo "$(ts) observing ${FAULT_SEC}s (RL controller mitigates via rate control)..."
sleep "${FAULT_SEC}"
