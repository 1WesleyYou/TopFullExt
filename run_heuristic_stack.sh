...
# Start/restart TopFull heuristic stack on master node:
# - proxy
...
# Ensure cAdvisor is ready before starting controller; otherwise CPU metrics may stay stale.
echo "[heuristic] Ensuring cAdvisor is ready on ${MASTER_TARGET}..."
bash "${SCRIPT_DIR}/ensure_cadvisor.sh"

# Sync runtime files to avoid stale remote scripts.
push_runtime_file "${MASTER_REPO_DIR}" "TopFull_master/online_boutique_scripts/src/deploy_heuristic.py"
...
ssh "${MASTER_TARGET}" bash -s -- \
  "${MASTER_REPO_DIR}" \
  "${NET_DELAY_MS:-0}" \
  "${EMPTYCART_DELAY_MS:-${NET_DELAY_MS:-0}}" \
  "${POSTCART_DELAY_MS:-${NET_DELAY_MS:-0}}" \
  "${NET_JITTER_MS:-0}" \
  "${NET_INJECT_AT_SEC:-0}" \
...
net_delay_ms="${2:-0}"
emptycart_delay_ms="${3:-0}"
postcart_delay_ms="${4:-0}"
net_jitter_ms="${5:-0}"
net_inject_at_sec="${6:-0}"
net_release_at_sec="${7:-0}"
...
tmux new-session -d -s topfull-controller "cd '${src_dir}' && PYTHONUNBUFFERED=1 python3 deploy_heuristic.py > /tmp/topfull-controller.log 2>&1"
tmux new-session -d -s topfull-metrics "cd '${src_dir}' && python3 metric_collector.py > /tmp/topfull-metrics.log 2>&1"

tmux ls
pgrep -af 'proxy_online_boutique|deploy_heuristic.py|metric_collector.py' || true
REMOTE
