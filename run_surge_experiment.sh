...
#   1:30  surge overlay begins (base continues)
#   3:30  surge ends automatically (-t flag)
#   5:30  experiment complete, all load stopped
#
# Usage:
...
echo "$(ts) BASE_RATE=${BASE_RATE}%  SURGE_RATE=${SURGE_RATE}%"
...
echo "$(ts) Phase 1/3: starting base load (RATE=${BASE_RATE}%), settling for ${BASE_SETTLE}s"
append_phase_ts "BASE_LOAD_START"
make -C "${SCRIPT_DIR}" inject-base RATE="${BASE_RATE}"
sleep "${BASE_SETTLE}"

echo ""
echo "$(ts) Phase 2/3: injecting surge (RATE=${SURGE_RATE}%) for ${SURGE_SEC}s (base continues)"
append_phase_ts "SURGE_START"
LOAD_RATE="${SURGE_RATE}" INJECT_DURATION="${SURGE_SEC}s" "${SCRIPT_DIR}/start_injection_surge.sh"
sleep "${SURGE_SEC}"
