...
class Agent:
    def __init__(self, target_apis, algo, interval=2, alpha=0.9, ttl=150, code='online_boutique'):
...
while True:
    ...
    for svc in overloaded_services:
        ...
        # Action: Create a new agent with the cluster
            ...
            # new_agent = Agent(cluster_apis, algo, interval=2)
            new_event = threading.Event()
            tid = threading.Thread(target=run_agent, args=(new_agent, new_event))
            tid.start()
