...
# ── Network-fault compensation config (from .env) ──
_net_comp_target = float(_dot_env.get("NET_COMPENSATE_TARGET", "0.85"))
_net_comp_cap    = float(_dot_env.get("NET_COMPENSATE_CAP", "1.30"))
_net_comp_slew   = float(_dot_env.get("NET_COMPENSATE_RECOVERY_SLEW", "0.20"))
_net_comp_fr     = float(_dot_env.get("NET_COMPENSATE_FAIL_RATIO", "0.10"))

print(f"[rl2 config] target={_net_comp_target}  cap={_net_comp_cap}  "
      f"recovery_slew={_net_comp_slew}  fail_ratio_threshold={_net_comp_fr}")


class NetCompensator:
    """Adaptive P-controller for network-fault compensation.

    Maintains a per-API baseline goodput measured during the healthy phase.
    During a network fault (detected via fail ratio):
      - compares current goodput to baseline * target_ratio
      - adjusts proxy thresholds proportionally to the gap (P-control)
    During recovery (fault cleared):
      - exponentially decays thresholds back to baseline values
    """

    def __init__(self, det, target_ratio=0.85, cap=1.30, recovery_slew=0.20):
        self.det = det
        self.target_ratio = target_ratio
        self.cap = cap
        self.recovery_slew = recovery_slew

        self._collector = Collector(code=global_config["microservice_code"])
        self._baseline = {}
        self._baseline_rps = {}
        self._active = False
        self._warmup_cycles = 0

    def _sample_goodput(self):
        metrics = self._collector.query()
        result = {}
        for api_name in self.det.apis:
            ...
            result[api_name] = (gp, rps, fr)
        return result

    def tick(self, fault_type):
        """Called every main-loop cycle. Returns True if compensation handled this cycle."""
        samples = self._sample_goodput()

        if fault_type == "none":
            if self._active:
                self._do_recovery(samples)
                return True
            self._update_baseline(samples)
            return False

        if fault_type == "network":
            self._do_compensate(samples)
            return True

        return False

    def _update_baseline(self, samples):
        for api, (gp, rps, fr) in samples.items():
            if rps < 1.0 or fr > 0.05:
                continue
            alpha = 0.3
            if api in self._baseline:
                self._baseline[api] = self._baseline[api] * (1 - alpha) + gp * alpha
                self._baseline_rps[api] = self._baseline_rps[api] * (1 - alpha) + rps * alpha
            else:
                self._baseline[api] = gp
                self._baseline_rps[api] = rps
        self._warmup_cycles += 1

    def _do_compensate(self, samples):
        if not self._baseline:
            print("[compensator] no baseline yet, skipping")
            return

        self._active = True
        updated = []

        # Phase 1: classify each API as affected or clean based on its own fail ratio
        affected = {}
        clean = {}
        for api_name in self.det.apis:
            s = samples.get(api_name)
            if s is None or api_name not in self._baseline:
                continue
            _, _, fr = s
            if fr > 0.05:
                affected[api_name] = s
        ...
        # Phase 2: affected APIs — gap-based P-control compensation
        for api_name, (current_gp, current_rps, fr) in affected.items():
            api_info = self.det.apis[api_name]
            target_gp = self._baseline[api_name] * self.target_ratio
            baseline_rps = self._baseline_rps[api_name]

            if current_gp >= target_gp or current_rps < 1.0:
                continue

            gap_ratio = (target_gp - current_gp) / target_gp if target_gp > 0 else 0
            boost = 1.0 + gap_ratio
            boost = min(boost, self.cap)
            new_thresh = baseline_rps * boost

            api_info['threshold'] = new_thresh
            updated.append(api_info)
            print(f"[compensator:affected] {api_name}: fr={fr:.2f} gp={current_gp:.0f} "
                  f"gap={gap_ratio:.2f} → threshold={new_thresh:.1f}")

        # Phase 3: clean APIs — boost threshold to absorb freed capacity
        total_lost_gp = 0.0
        for api_name, (gp, rps, fr) in affected.items():
            total_lost_gp += max(self._baseline.get(api_name, 0) - gp, 0)

        if clean and total_lost_gp > 0:
            share = total_lost_gp / len(clean)
            for api_name, (current_gp, current_rps, fr) in clean.items():
                api_info = self.det.apis[api_name]
                baseline_rps = self._baseline_rps[api_name]
                extra = min(share, baseline_rps * (self.cap - 1.0))
                new_thresh = baseline_rps + extra
                api_info['threshold'] = new_thresh
                updated.append(api_info)
                print(f"[compensator:clean]    {api_name}: fr={fr:.2f} → "
                      f"threshold={new_thresh:.1f} (+{extra:.1f} absorb)")

        if updated:
            apply_threshold_proxy(updated)
        else:
            print("[compensator] all APIs meet target, holding")

    def _do_recovery(self, samples):
        all_recovered = True
        updated = []

        for api_name, api_info in self.det.apis.items():
            if api_name not in self._baseline_rps:
                continue
            baseline_rps = self._baseline_rps[api_name]
            current_thresh = api_info['threshold']

            if current_thresh <= baseline_rps * 1.02:
                api_info['threshold'] = baseline_rps
                continue

            all_recovered = False
            new_thresh = current_thresh - (current_thresh - baseline_rps) * self.recovery_slew
            api_info['threshold'] = max(new_thresh, baseline_rps)
            updated.append(api_info)
            print(f"[compensator:recovery] {api_name}: threshold {current_thresh:.1f} → {new_thresh:.1f} "
        ...
        if all_recovered:
            print("[compensator] fully recovered, deactivating")
            self._active = False


_compensator = NetCompensator(
    detector,
    target_ratio=_net_comp_target,
    cap=_net_comp_cap,
    recovery_slew=_net_comp_slew,
)
...
while True:
    ...
    # Detect overload (with fail-ratio signal enabled)
    overloaded_services = detector.detect(0.8, fail_ratio_threshold=_net_comp_fr)

    # Adaptive network-fault compensation
    handled = _compensator.tick(detector.last_fault_type)
    if handled:
        continue
